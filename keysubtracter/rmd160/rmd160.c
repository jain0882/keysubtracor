...
RMDcompress (uint32_t * MDbuf, uint32_t * X, ThreadPool* pThreadPool)
{
  ...
  ThreadArgs* pThreadArgRoundOne = (ThreadArgs *)malloc(sizeof(ThreadArgs));
  ThreadArgs* pThreadArgRoundTwo = (ThreadArgs *)malloc(sizeof(ThreadArgs));
  
  sem_t semaphore;
  sem_init(&semaphore, 0, 0);

  pThreadArgRoundOne->pMDbuf = MDbuf;
  pThreadArgRoundOne->pX = X;
  pThreadArgRoundOne->pSemaphore = &semaphore;
  ...
  pThreadArgRoundTwo->pX = X;
  pThreadArgRoundTwo->pSemaphore = &semaphore;
...
/* combine results */
  ...
  MDbuf[1] = MDbuf[2] + pThreadArgRoundOne->d + pThreadArgRoundTwo->e;
  MDbuf[2] = MDbuf[3] + pThreadArgRoundOne->e + pThreadArgRoundTwo->a;
